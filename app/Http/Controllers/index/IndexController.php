...
class IndexController extends Controller
{
    ...
    public function prolist_do(){
        ...
        $one_goods =$mem->get('IndexController_proindo');
        if (empty($goods)) {
            $one_goods =Goods_goods::get();
            $mem->set('IndexController_proindo',$one_goods, 0,10);
        }
    ...
    public function proinfo($id){
        ...
        $mem = new \memcache;
        $mem->connect('127.0.0.1','11211');
        $goods =$mem->get('IndexController_proinfo');
        if (empty($goods)) {
            $goods =Goods_goods::find($id);
            $mem->set('IndexController_proinfo',$goods, 0,10);
        }
    ...
    public function pay(){
        ...
        $ids =request()->ids;
        $idss =explode(',',$ids);
        $pay =Car::whereIn('goods_id',$idss)->get();
        //查询收获地址
